...
class ALIKED(nn.Module):
    ...
    def __init__(
        ...
        if load_pretrained:
            ...
            pretrained_path = osp.join('pretrained', f'{model_name}.pth')
    ...
    def forward_score(self, image):
        torch.cuda.synchronize()
        _, score_map = self.extract_dense_map(image)
        return score_map
    
    def forward_feature_map(self, image):
        torch.cuda.synchronize()
        feature_map, _ = self.extract_dense_map(image)
        return feature_map
