...
class U1652DatasetTrain(Dataset):
    ...
    def __init__(self,
        ...
        for idx in self.ids:
            if len(self.query_dict[idx]["files"]) == 1:
                ...
                for g in gallery_imgs:
                    for q in query_imgs:
                        self.pairs.append((idx, label, "{}/{}".format(query_path, q), "{}/{}".format(gallery_path, g)))
        ...
        self.train_multi_weather = train_multi_weather

        if train_multi_weather:
    ...
    def __getitem__(self, index):
        ...
        if self.train_multi_weather == True:
            idx_weather = random.choice(range(len(self.iaa_weather_list)))
            # print(idx_weather)
            if idx_weather == 0:
                gallery_img = gallery_img
            else:
                # for drone
                gallery_img = self.iaa_weather_list[idx_weather](image=gallery_img)
...
class U1652DatasetEval(Dataset):
    ...
    def __init__(self,
        ...
        if self.weather_idx != None:
            self.aug = iaa_weather_list[self.weather_idx]
        else:
            self.aug = None
