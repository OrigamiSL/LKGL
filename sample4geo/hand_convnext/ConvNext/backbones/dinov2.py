...
class DinoV2_self(nn.Module):
    ...
    def __init__(self, model_name: _DINO_V2_MODELS, layer1: int = 39,  facet1: _DINO_FACETS = "value", use_cls=False,
        ...
        if 'vitg14' in self.model_name:
            ...
            self.dino_model = torch.hub.load(r'./sample4geo/hand_convnext/ConvNext/backbones/facebookresearch_dinov2_main/dinov2', self.model_name, pretrained=False, trust_repo=True, source='local')  
            # 加载DINOv2模型
            self.dino_model.load_state_dict(torch.load(r'./pretrained/dinov2_vitb14_pretrain.pth'))
            # 加载本地权重
            if self.layer1 > 11:
