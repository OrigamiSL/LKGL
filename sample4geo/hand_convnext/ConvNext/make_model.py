...
class build_convnext(nn.Module):
    def __init__(self, num_classes, block=4, return_f=False, resnet=False, pos_scale=0.1,  backbone = None):
        ...
        if resnet:
            ...
            if backbone == 'conv':
                convnext_name = "convnext_base_w_score"
            elif backbone == 'dino':
                convnext_name = "dinov2"
    ...
    def forward(self, x, x_score = None, wo_KGFPN = False):
        ...
        gap_feature, part_features = self.convnext(x, x_score, wo_KGFPN)
        # part_features = part_features + self.pos_embed * self.pos_scale

        # -- Training
        if self.training:

            return None, None, gap_feature, part_features
...
def make_convnext_model(num_class, block=4, return_f=False, resnet=False, pos_scale=0.1,  backbone = None):
    print('===========building convnext===========')
    model = build_convnext(num_class, block=block, return_f=return_f, resnet=resnet, pos_scale=pos_scale,  backbone =  backbone)
    return model
