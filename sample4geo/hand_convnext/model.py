...
class two_view_net(nn.Module):
    def __init__(self, class_num, block=4, return_f=False, resnet=False, pos_scale=0.1, if_learn_ECE_weight=False,
        ...
        self.model_1 = make_convnext_model(num_class=class_num, block=block, 
                                           return_f=return_f, resnet=resnet, pos_scale=pos_scale,
                                            backbone =  backbone)
        
        self.wo_KGFPN = wo_KGFPN
    ...
    def forward(self, x1, x2=None, score_map_q = None, score_map_r = None):

        if x2 is not None and score_map_q is not None:
            y1 = self.model_1(x1, score_map_q, self.wo_KGFPN)
            y2 = self.model_1(x2, score_map_r, self.wo_KGFPN)
            return y1, y2
        else: ## test
            y1 = self.model_1(x1, x2, self.wo_KGFPN)
            return y1
